...
type RestOf<T extends ITask | NewTask> = Omit<T, 'startDate' | 'endDate' | 'createdAt' | 'updatedAt'> & {
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      endDate: task.endDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      endDate: restTask.endDate ? dayjs(restTask.endDate) : undefined,
